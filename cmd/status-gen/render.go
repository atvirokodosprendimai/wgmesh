package main

import (
	"fmt"
	"sort"
	"strings"

	"github.com/atvirokodosprendimai/wgmesh/eidos/eidosmeta"
)

var matrixDimensions = []string{"api", "behavior", "cli", "wire"}

type FeatureStatus struct {
	Name          string
	Slug          string
	Status        eidosmeta.Status
	TrackingIssue string
	Since         string
	Dimensions    []DimensionStatus
}

type DimensionStatus struct {
	Name    string
	Present bool
	Note    string
}

func RenderStatus(features []FeatureStatus) string {
	features = append([]FeatureStatus(nil), features...)
	sort.Slice(features, func(i, j int) bool {
		if features[i].Name == features[j].Name {
			return features[i].Slug < features[j].Slug
		}
		return features[i].Name < features[j].Name
	})

	var b strings.Builder
	b.WriteString("<!-- DO NOT EDIT: generated by `make status` -->\n")
	b.WriteString("# Feature Ledger Status\n\n")
	b.WriteString("## Matrix\n\n")
	b.WriteString("| Feature | Status | Tracking | Since | API | Behavior | CLI | Wire |\n")
	b.WriteString("| --- | --- | --- | --- | --- | --- | --- | --- |\n")

	stats := statusStats{features: len(features)}
	var provisionalGaps []string
	var missingArtifactGaps []string
	for _, feature := range features {
		switch feature.Status {
		case eidosmeta.StatusImplemented:
			stats.implemented++
		case eidosmeta.StatusProvisional:
			stats.provisional++
		}
		if isProvisionalOrImplementable(feature.Status) {
			provisionalGaps = append(provisionalGaps, fmt.Sprintf("- %s: %s", feature.Name, feature.Status))
		}

		byDimension := map[string]DimensionStatus{}
		for _, dimension := range feature.Dimensions {
			byDimension[dimension.Name] = dimension
			stats.claimed++
			if dimension.Present {
				stats.satisfied++
			} else {
				stats.missing++
				missingArtifactGaps = append(
					missingArtifactGaps,
					fmt.Sprintf("- %s / %s: %s", feature.Name, dimension.Name, dimension.Note),
				)
			}
		}

		b.WriteString("| ")
		b.WriteString(escapeTableCell(feature.Name))
		b.WriteString(" | ")
		b.WriteString(string(feature.Status))
		b.WriteString(" | ")
		b.WriteString(renderTrackingIssue(feature.TrackingIssue))
		b.WriteString(" | ")
		b.WriteString(renderSince(feature.Since))
		for _, dimension := range matrixDimensions {
			status := "-"
			if entry, ok := byDimension[dimension]; ok {
				if entry.Present {
					status = "OK"
				} else {
					status = "MISSING"
				}
			}
			b.WriteString(" | ")
			b.WriteString(status)
		}
		b.WriteString(" |\n")
	}

	b.WriteString("\n## Gap Roster\n\n")
	b.WriteString("### Provisional / Implementable Features\n\n")
	if len(provisionalGaps) == 0 {
		b.WriteString("No provisional or implementable features.\n")
	} else {
		sort.Strings(provisionalGaps)
		for _, gap := range provisionalGaps {
			b.WriteString(gap)
			b.WriteString("\n")
		}
	}
	b.WriteString("\n### Missing Compat Artifacts\n\n")
	if len(missingArtifactGaps) == 0 {
		b.WriteString("No missing claimed dimensions.\n")
	} else {
		sort.Strings(missingArtifactGaps)
		for _, gap := range missingArtifactGaps {
			b.WriteString(gap)
			b.WriteString("\n")
		}
	}

	b.WriteString("\n## Summary\n\n")
	b.WriteString(fmt.Sprintf("- Features: %d\n", stats.features))
	b.WriteString(fmt.Sprintf("- Implemented features: %d\n", stats.implemented))
	b.WriteString(fmt.Sprintf("- Provisional features: %d\n", stats.provisional))
	b.WriteString(fmt.Sprintf("- Claimed dimensions: %d\n", stats.claimed))
	b.WriteString(fmt.Sprintf("- Satisfied dimensions: %d\n", stats.satisfied))
	b.WriteString(fmt.Sprintf("- Missing dimensions: %d\n", stats.missing))

	return b.String()
}

type statusStats struct {
	features    int
	implemented int
	provisional int
	claimed     int
	satisfied   int
	missing     int
}

func escapeTableCell(value string) string {
	return strings.ReplaceAll(value, "|", "\\|")
}

func isProvisionalOrImplementable(status eidosmeta.Status) bool {
	return status == eidosmeta.StatusProvisional || status == eidosmeta.StatusImplementable
}

func renderTrackingIssue(issue string) string {
	issue = strings.TrimSpace(strings.TrimPrefix(issue, "#"))
	if issue == "" {
		return "-"
	}
	return fmt.Sprintf("[#%s](https://github.com/atvirokodosprendimai/wgmesh/issues/%s)", issue, issue)
}

func renderSince(since string) string {
	if since == "" {
		return "-"
	}
	return escapeTableCell(since)
}
